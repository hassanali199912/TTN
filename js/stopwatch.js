    ...
    timers_buttons.forEach((element) => {
        if (element.classList.contains("btn-active")) {
            if (element.getAttribute("data-timer") == "custom") {
                ...
                }else if(timer_input_minutes.value < 0 || timer_input_seconds.value < 0){
                    alert("Don't Enter Ngative Value or zero, Please Enter Right Value To Start");
...
function timer_initial() {
    ...
    timer_input_minutes.value = timer_input_minutes.value.length < 2 ? "0" + timer_input_minutes.value : timer_input_minutes.value;


    timer_input_seconds.value = timer_input_seconds.value < 10 ? "0" + timer_input_seconds.value : timer_input_seconds.value;
}
